...
async def admin(message):
     if message.chat.id == config.admin:
        await bot.send_message(message.chat.id, f"*Привет великий \nАдмин Тони*",  reply_markup=keyboard.admin, parse_mode='Markdown')
...
async def get_message(message):
    ...
    if message.text == "Ꮯʙяɜᴀᴛьᴄя ᴄ ʍᴇнᴇджᴇᴩᴏʍ дᴧя ᴧичнᴏй ᴋᴏнᴄуᴧьᴛᴀции📈":
    ...
    if message.text == "Адʍинᴄᴋиᴇ ᴋᴏнɸиᴦуᴩᴀции👩‍💻":
        await bot.send_message(message.chat.id, text = f"*Управляй ботом, сэээр!*", reply_markup=keyboard.static, parse_mode='Markdown')

    if message.text == "Измени новости🏪":
        await bot.send_message(message.chat.id, text = f"*Создай новость, сэээр, Тони! Команда: /me*", parse_mode='Markdown')

    if message.text == "Нᴏʙᴏᴄᴛи🏪":
    ...
    if message.text == "Ꭹᴄᴛᴀнᴏʙиᴛь ᴄᴏᴇдинᴇниᴇ📲":
        ...
        await bot.send_message(message.chat.id, text = f"*Повторите попытку или вернитесь назад /menu*", reply_markup=keyboard.connect, parse_mode='Markdown')  
...
async def adminone(call: types.CallbackQuery):
    if call.message.chat.id == config.admin:
        ...
        await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f"*Вот статистика уважаемый Тони: \n*{d}* человек*", parse_mode='Markdown')
    else:
